...
`@actions/ファイル名.md` で呼び出すことで、確認の往復をゼロにします。

### 向いているケース / 向いていないケース

| ✅ 有効 | ❌ 向いていない |
|--------|---------------|
| 繰り返し実行するタスク | 一度きりのタスク |
| 手順が決まっているタスク | 手順が毎回変わるタスク |
| 複数ステップがあるタスク | 探索的・創造的なタスク |

### 新しい action の追加

1. `docs/actions/` に `新しいタスク名.md` を作成
2. 以下の構造で記述：
   ...
   理解できましたか？ゴーサインをください。
   ---
   1. ステップ1（必要に応じてコマンドブロックを使う）
   2. ステップ2
   ```

3. `docs/actions/README.md` と `help.md` のリストに追加

### 実測データ

joplin-ai-tools プロジェクト（2025年10月）での計測：平均 **69% トークン削減**。
詳細は `docs/notes/2025-10-25-00-00-00-actions-pattern-rationale.md` を参照。
