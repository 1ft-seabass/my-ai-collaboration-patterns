...
## 🔄 v1 からの移行（husky → simple-git-hooks）

> **🤖 AIへのワンショット指示（コピペ用）**
>
> ```
> https://github.com/1ft-seabass/my-ai-collaboration-patterns/patterns/setup-pattern/setup-securecheck
> setup-securecheck v1（husky + lint-staged）から v2（simple-git-hooks）に移行したいです。
>
> npx degit 1ft-seabass/my-ai-collaboration-patterns/patterns/setup-pattern/setup-securecheck/migration ./tmp/securecheck-migration
>
> tmp/securecheck-migration/MIGRATION_GUIDE_v1_to_v2.0.0.md を読んで、移行を案内してください。
> ```
...
└── templates/
    ...
    └── scripts/
        ...
        └── pre-commit.js             # Node.js 版 pre-commit フック（ログ出力あり）
```
...
- package.json の必須 npm scripts

**中身チェック（3項目）**:
- .secretlintrc.json に preset-recommend が含まれているか
- gitleaks.toml に検出ルール（[extend] または [[rules]]）があるか（[allowlist] のみだと検出ルール0個で動作するため）
- .git/hooks/pre-commit に pre-commit.js の記述があるか

**動作チェック（6項目）**:
- secretlint コマンドが動くか
- gitleaks バイナリが動くか
- gitleaks 機能的カナリアテスト（合成シークレットを実際に検出できるか）
- 実行ログの最終確認（.logs/pre-commit.log）
- ネガティブテスト実行痕跡（ステップ3.6.5のネガティブテストが実際に実行されカナリアがブロックされた記録があるか）
- 毎コミット自動カナリア自己検証の実行痕跡（pre-commit.js が毎コミット自動でカナリアを注入し、検出器が生きているかを直近コミットで確認できているか）
...
### 毎コミット自動カナリア自己検証
「検出ルールが今も生きているか」を人間が意識せず**毎コミット自動で**証明する仕組み。ステップ3.6.5の手動ネガティブテストとは別物で、こちらは追加のプロセス起動なしに動く:
...
- 結果は `.logs/pre-commit.log` の `autoCanary: { gitleaks, secretlint }` フィールド（各 `ok` / `failed` / `cleanup-failed` / `skipped`）に記録され、check#14 がこれを確認する
