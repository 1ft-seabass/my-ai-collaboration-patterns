...
// security-verify.js check#14 がこの値をログの autoCanary フィールドとして参照する。
let secretlintSelfCheck = 'skipped';
let gitleaksSelfCheck = 'skipped';

// 後片付けが本当に必要な状態かどうかを覚えておくフラグ。
// pre-commit.js は複数箇所で早期に失敗しうるため、process.exit() を直接呼ばず
// 必ずこの下の try/finally を1回通してから最後にまとめて exit する構造にしている。
