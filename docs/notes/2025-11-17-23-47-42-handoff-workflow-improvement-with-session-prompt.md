---
tags: [letters, workflow, session-prompt, ai-instruction]
---
...
### 4. 追加改善（2025-11-16 第2セッション）

実際の運用テストで判明した課題と対応：

**課題1: AIが「質問」してしまう問題**
- 現象: 「コミットログ方針を教えてください」とAIが質問
- 期待: AIが方針を考えて提案し、人間が承認する形
- 対応: ステップ3,5,10を「相談」から「提案」に変更

**課題2: 申し送り作成完了通知が不完全**
- 現象: ファイルパスのみの1行出力になることがある
- 期待: 完全なフォーマットで次セッション用定型文を提示
- 対応: ステップ12に「重要: 上記の完全なフォーマットで出力すること（ファイルパスのみの出力は不可）」を追加

**課題3: セッション途中で勝手にノート・申し送りを作成**
- 現象: 運用ルールを知っているがゆえに、指示されていないタイミングでノート・申し送りを作成
- 期待: 明示的に指示されたときのみ実行
- 対応: 運用ルールに実行タイミングの注意書きを追加
  ```
  ・知見は docs/notes/ (...) ※注意: ノート作成はセッション終了時の申し送りフロー内でのみ実行すること
  ・申し送りは docs/letters/ (...) ※注意: 申し送り作成はユーザーが明示的に指示したときのみ実行すること
  ```
- Markdownリスト記号を `-` から `・` (中黒) に変更（レンダリングの安定性向上）

**課題4: writing-collaborate パターンに actions がない**
- 対応: `patterns/writing-collaborate/templates/actions/` ディレクトリを新規作成し、改善版の `current_create_letter.md` を追加

## 今後の改善案

### 短期
- ~~実際の運用でこの定型文がどれだけ機能するかをモニタリング~~ → 完了（第2セッションで実施）
